...
function AllInternships() {
	...
	const fetchInternship = async (page: number, size: number, sort: string) => {
		try {
			const response = await axios.get(`/api/internship/supervisor`, {
				params: {
					pageNo: page,
					limit: size,
					sortBy: sort,
			...
			const { content, totalPages } =
				response.data as PageableResponse<Internship>;
	...
	return (
		<div className="bg-white p-5 rounded-md w-full pt-0">
			<div className="flex items-center justify-between pb-6">
				<div>
					<h1 className="font-bold text-4xl font-semibold mb-4">
						Stajlar
					</h1>
							...
							{internships.map((internship: Internship) => (
								<tr key={internship.id}>
										...
										<div className="flex items-center">
											<div className="ml-3">
												<p className="text-gray-900 whitespace-no-wrap">
													{internship.facultySupervisor.name +
														" " +
														internship.facultySupervisor.surname}
												</p>
											</div>
										</div>
									</td>
									<td
										...
										onClick={() =>
											openStudentModal(internship.student)
										}
									>
									...
									<td
										className="px-5 py-5 border-b border-gray-200 bg-white text-sm"
										onClick={() =>
											openCompanyModal(internship.company.id)
										}
									>
										<p className="border border-blue-500 text-blue-500 rounded-md px-4 py-2 transition duration-300 ease-in-out hover:bg-blue-500 hover:text-white cursor-pointer text-center">
											<span className="inline-block w-40 mx-auto">
									...
									<td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
										...
										<button
											...
											onClick={() =>
												openDocumentModal(internship.id)
											}
										>
											Görüntüle
										</button>
			...
			{showDocumentModal && (
				<div className="fixed inset-0 flex items-center justify-center z-50">
					...
					<div className="bg-white rounded-lg p-8 z-50">
						<InternshipDocument
							_internshipId={selectedInternshipId}
