...
function FacultySupervisorProfile() {
    const [facultySupervisorDatas, setFacultySupervisorDatas] = useState<FacultySupervisor[] | null>(null);
    ...
    useEffect(() => {
        fetch('/api/user/faculty-supervisor/auth', {
            ...
            .then(data => {
                ...
                fetch('/api/facultySupervisor/byUserId/' + facultySupervisorData.id, {
                    method: 'GET'
                  })
                    .then(response => response.json())
                    .then(data => {
                      ...
                      fetch('/api/faculty/' + data.facultyId, {
                        method: 'GET'
                      })
                        .then(response => response.json())
                        .then(facultyData => {
                    ...
                    facultySupervisorDatas.map(facultySupervisorData => (
                        <div key={facultySupervisorData.email} className="grid grid-cols-2 gap-4 px-2 w-full">
                            ...
                            <div className="flex flex-col justify-center rounded-2xl bg-white bg-clip-border px-3 py-4 shadow-3xl shadow-shadow-500">
                                ...
                                <p className="text-base font-medium text-navy-700">
                                    {facultySupervisorData.createDate.toLocaleDateString()}</p>
                            </div>
                        </div>
